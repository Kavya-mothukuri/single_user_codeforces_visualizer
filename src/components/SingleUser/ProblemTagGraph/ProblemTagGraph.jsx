...
const ProblemTagGraph = ({userSubmissions}) => {
  ...
  for(let i=0;i<userSubmissions.length;i++)
  {
    ...
    id.set(key,true);
    let currentTag=userSubmissions[i].problem.tags;
    for(let j=0;j<currentTag.length;j++)
    {
      if(!tag.has(currentTag[j]))
      {
        tag.set(currentTag[j],1);
      }
      else
      {
        let prevCnt=tag.get(currentTag[j]);
        tag.set(currentTag[j],prevCnt+1);
      }
    }
  }
